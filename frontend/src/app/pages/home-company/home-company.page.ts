...
export class HomeCompanyPage implements OnInit {
  ...
  getCompanyName(): string {
    ...
    if (
      ...
      companyNames[this.currentUser.institutionId as keyof typeof companyNames]
    ) {
      return companyNames[
        this.currentUser.institutionId as keyof typeof companyNames
      ];
    }
  ...
  getExpirationLabel(coupon: Coupon): string {
    ...
    const daysLeft = Math.floor(
      (expirationDate.getTime() - this.currentDate.getTime()) /
        (1000 * 3600 * 24)
    );
  ...
  getCouponScoreLabel(coupon: Coupon): string {
    return coupon.scoreRequired
      ? `${coupon.scoreRequired} pontos`
