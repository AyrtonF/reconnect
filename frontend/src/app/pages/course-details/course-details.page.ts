...
export class CourseDetailsPage implements OnInit {
  ...
  private checkForQuizAchievement(score: number) {
    ...
    if (score === 100) {
      ...
      this.courseService
        .addAchievement(this.courseId, achievement)
  ...
  private calculateQuizScore(
    ...
    const totalPoints = this.currentQuiz.questions.reduce(
  ...
  private checkForAchievements() {
    ...
    const allVideosWatched = this.currentModule.content.videos.every(
      (v) => v.isWatched
    );
    if (allVideosWatched) {
      const achievement: Achievement = {
